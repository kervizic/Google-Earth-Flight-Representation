...
refreshInterval # String
    ...
    # not used here
    #def addMultiGeometry (self, name, description, points, altitude, visibility, color) :
        #"adds the line to a kml file"
        #self.openPlacemark(name , description, visibility)
        #self.kml += self.ind + '<Style>\n'
        #self.indMore()
        #self.addLineStyle('FF' + str(color[2:]), '0.5')
        #self.addPolyStyle (color)
        #self.indLess()
        #self.kml += self.ind + '</Style>\n'
        #self.kml += self.ind + '<MultiGeometry>\n'
        #self.indMore()
        #polyPoints = {}
        #for i in xrange( len(points) - 1 ) :
            #polyPoints[0] = (
                #str(points[i].coordinate['longitude']) + ',' +
                #str(points[i].coordinate['latitude']) + ',' +
                #str(altitude['min'])
                #)
            #polyPoints[1] = (
                #str(points[i+1].coordinate['longitude']) + ',' +
                #str(points[i+1].coordinate['latitude']) + ',' +
                #str(altitude['min'])
                #)
            #polyPoints[2] = (
                #str(points[i+1].coordinate['longitude']) + ',' +
            ...
            #polyPoints[3] = (
                ...
                #str(altitude['max'])
                #)
            #add = self.addPolygon(polyPoints, 'absolute')
        #polyPointsTop = {}
        #for i in xrange( len(points) - 1 ) :
            #polyPointsTop[i] = (
                #str(points[i].coordinate['longitude']) + ',' +
                ...
                #str(altitude['max'])
                #)
        #add = self.addPolygon(polyPointsTop, 'absolute')
        #polyPointsBottom = {}
        #for i in xrange( len(points) - 1 ) :
            #polyPointsBottom[i] = (
                #str(points[i].coordinate['longitude']) + ',' +
                ...
                #str(altitude['min'])
                #)
        #add = self.addPolygon(polyPointsBottom, 'absolute')
        #polyPointsBottomClamp = {}
        #for i in xrange( len(points) - 1 ) :
            #polyPointsBottomClamp[i] = (
                #str(points[i].coordinate['longitude']) + ',' +
                #str(points[i].coordinate['latitude']) + ',' +
                #str(altitude['min'])
                #)
        #add = self.addPolygon(polyPointsBottomClamp, 'clampToGround')
        #self.indLess()
        #self.kml +=(self.ind + '</MultiGeometry>\n')
        #self.closePlacemark()
